...
metric = WLAN aggregate DL throughput (Mbps)
  ...
  metric >= TH_HIGH (default 500 Mbps)  ─► gNB2 = ACTIVE       (WLAN 포화 → 오프로드 불가)
  metric <= TH_LOW  (default 300 Mbps)  ─► gNB2 = DEEP_SLEEP   (WLAN 여유 → 오프로드 가능)
  TH_LOW < metric < TH_HIGH             ─► gNB2 = previous_state  (밴드 내 유지)
```

**방향 근거**: WLAN 측 throughput이 높다는 건 Non-3GPP 경로가 이미 바쁘다는 뜻 → gNB2의 트래픽을 흡수할 여유가 없음 → gNB2를 ACTIVE로 유지해야 함. 반대로 WLAN이 한가하면 여유 capacity가 있으므로 gNB2를 재워서 트래픽을 Wi-Fi로 오프로드 가능. 이 논리는 `tools/decide_cell_power.py`의 R4(spare-capacity)와 동일.
...
**튜닝 가이드** (방향 뒤집힌 룰 기준):
- 둘 다 크게 (예: 800/600) → ACTIVE 진입 조건이 까다로움 → 평시 DEEP_SLEEP 비율↑ (공격적 에너지 절감, 단 Wi-Fi 살짝 바빠도 gNB2 잠듦)
- 둘 다 작게 (예: 200/100) → ACTIVE 쉽게 진입 → 평시 ACTIVE 비율↑ (보수적, 에너지 절감 적음)
- 간격 크게 (예: 800/200) → 한 번 전환되면 잘 안 바뀜 (느린 응답, 안정성↑)
...
| 상황 | 동작 | 비고 |
|---|---|---|
| `userDataUsageMeasurements` 비어있음 | metric=0 → `0 ≤ TH_LOW` → **DEEP_SLEEP 결정**. 측정 데이터가 없는데 셀을 재우는 건 안전상 위험 → cold-start 또는 missing-data 가드(예: 직전 N 샘플 이내 결정 보류, 또는 강제 ACTIVE)를 ModuTec 구현에서 추가 권장 | log: `metric=0.0` |
...
| 이름 | 기본값 | 의미 |
|---|---|---|
| `TH_HIGH_MBPS` | `500.0` | 이 이상이면 **ACTIVE** 결정 (WLAN 포화 → 오프로드 불가) |
| `TH_LOW_MBPS` | `300.0` | 이 이하이면 **DEEP_SLEEP** 결정 (WLAN 여유 → 오프로드 가능) |
